# HTTP-SERVER-CL004 设计 v1.2 — 端口探测「假占用」根因 + CL003 遗留三项

- 件号：`documents/http-server-port-residual-design-v1.2-20260922.md`
...
- 评审链：v1.0 `6edb021` → **CONDITIONAL_PASS 90/100**（F-1/F-2 必改 + R-1~R-7）→ v1.1 `3b0d37a` → **CONDITIONAL_PASS 93/100**（F-1 闭合、R-1~R-7 落实；F-2 残留 (a)(b)(c) 必改 + R-8/R-9）→ **v1.2（本版）**
- 依据来源：CL003 设计 §10 O1 + CL003 审计 AUD-1/AUD-2/AUD-3

## 0 v1.2 修订落点表

### 0.1 第二轮（v1.1 → v1.2，对 rereview 93/100）
...
> **我方 runner 归属规则（显式定义）**：某 pid 的 `ps -o args=` 命令行**同时**含 `runner.py` 且含该 tmpdir 路径 ⇒ 计为该目录的服务进程。
...
| **A4**（F-2 三轮口径） | 同目录 100ms 内连续两次 `hs <tmpdir> -d --url`；随后采集：① `L = lsof -nP -iTCP -sTCP:LISTEN -F p` 的**全部 LISTEN pid**；② 按归属规则过滤出 `O`（命令行含 `runner.py` 且含 `<tmpdir>` 的 pid）；③ `R = json.load(~/.http-server.cli/registry.json)` 中 `path == <tmpdir>` 的条目、`R_pids = {e['pid'] for e in R}` | 修后：`len(R) == 1` **且** `O == R_pids == {该条目 pid}`（**同一性**，非计数）；修前（worktree）反证：`O != R_pids`（可见形态：孤儿在**另一端口**，`O ⊋ R_pids`；隐形形态：`R_pids == ∅` 而 `O ≠ ∅`） |
| A5 | `hs -i index.html -p <port> -d --url <dir>`（CWD 下同名文件存在） | 服务落 **`<dir>`**（读 registry.json 该 path 条目核对）且 index_page 生效；修前反证：落 CWD |
...
| **A11**（F-1 + R-8 recipe） | ① 备份 registry.json；② 手工注入死条目 `{"port": <空闲端口如 8093>, "path": "<tmpdir>", "pid": 999999, "started_at": "..."}`；③ `hs <tmpdir> --json`；④ 还原备份 | stdout 首个非空字符 `{` 且 `json.loads` 成功；stderr 含 `Found stale registry entry`；退出码 0 |
