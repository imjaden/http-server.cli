#!/bin/bash
# scripts/release-local.sh — 本地安装脚本
# 用法: bash scripts/release-local.sh [选项]
...
case "${1:-}" in
    ...
    --versions)
        ...
        ;;
    --help|-h|"")
        echo "scripts/release-local.sh — 本地安装脚本"
        echo ""
        echo "用法:  bash scripts/release-local.sh [选项]"
        echo ""
        echo "选项:"
    ...
    *)
        echo "❌ 未知选项: $1"
        echo "用法: bash scripts/release-local.sh --help"
        exit 1
        ;;
esac
